...
class CustomPPOTrainer(PPOTrainer, Trainer):
    ...
    def get_rewards(
        ...
        # values: torch.Size([2, 208])----------------------------------
        if self.finetuning_args.using_reward_difference == True and values.size(0) % 2 == 0:

            coef_lambda = self.finetuning_args.coef_lambda
            
            for i in range(0,values.size(0)-1,2):
                q_0 = rewards[i]
                q_1 = rewards[i+1]
                diff = abs(q_0 - q_1)
                rewards[i] = rewards[i] - coef_lambda * diff
                rewards[i+1] = rewards[i+1] - coef_lambda * diff
        # ------------------------------------------------------------
